from modules import get_country, deg2num, num2deg, get_repr_locs,get_repr_locs_gpkg, get_repr_locs_adm1,get_repr_locs_adm1_gpkg
...
if __name__ == "__main__":
    ...
    config = utils.read_config("./config_ours.ini")
    ...
    if area_dict:
        force_update = False
        if ccode == 'PRK':
            con_loc_dict, geom_dict = get_repr_locs_gpkg(ccode, "./NK_adm2_24cities.gpkg")
        else:
            con_loc_dict, geom_dict = get_repr_locs(ccode)
        ...
        if ccode == 'PRK':
            loc_adm1_dict = get_repr_locs_adm1_gpkg(con_loc_dict)
        else:
            loc_adm1_dict = get_repr_locs_adm1(con_loc_dict)
    ...
    # Country to Geometry
    if create_geometry:
        result_list = country2list(ccode)
        # result_list = gpkg2list("./NK_adm2_24cities.gpkg")
        print(result_list)
        # time.sleep(1000)
