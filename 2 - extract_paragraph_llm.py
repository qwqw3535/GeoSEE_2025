...
parser.add_argument('--model', default=True, help='what llms will use')
parser.add_argument('--config_path', default='./config_ours.ini', help='config file location')
# parser.add_argument('--timeline', default=True, help='what timeline will use')
...
elif target_var == 'wo_neighbor':
    s_modules_path = f'./selected_modules/overall_{target_ccode}_{target_var}.json'
else:
    s_modules_path = f'./selected_modules/llms_poi_air/{target_ccode}_{target_var}_{model}.json'
...
if adm=='2':
    ...
    for area_id, loc in tqdm(area_dict.items()):
        ...
        for module in s_modules:
            module = module.replace(')', ', config=config)') 
            ...
            if 'get_aggregate_neighbor_info' in module:
                ...
                module_func = 'lambda x:'+module.replace('Loc','loc=x, config=config')
        ...
        for fname, desc in result_dict.items():
            for key, info in desc.items():
                if info:
                    # print(info)
                    if isinstance(info['val'], (list, tuple)):
                        vals_str = ", ".join(map(str, info['val']))
                        paragraph += f"{info['desc']} are {vals_str}"
        ...
        df.to_csv(f'./{target_ccode}_{model}_{target_var}{adm_offset}_{timeline}_paragraph_output.csv', index=False, header=True)
else:
    ccode_loc_path = proxy_dir / f'{target_ccode}_loc{adm_offset}.pickle'
    with open(ccode_loc_path,'rb') as f:
        area_dict = pickle.load(f)
