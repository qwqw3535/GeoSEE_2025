...
def query_gpt_openai(text_list, client, model, temperature=0,  max_completion_tokens=3000, max_try_num=10, tqdm_disable=False):
...
def get_prompt_for_extracting_modules(question, version='1'):
    with open("./templates/function_desc_NK.txt", "r") as f:
...
def get_prompt_for_inference_in_context(df, df_record, index, X_train, y_train, X_test, target_question, target_ccode=False, addr_only = False):        
    template = "<EXAMPLES>\n\n<TARGET>"
    ...
    example_desc = '\n'.join(['. '.join(df[df['area_id'] == row['area_id']].iloc[0].desc.split('\n')) + f'.\nAnswer: ' + str(round(row.score, 3)) + ' Million USD' for _, row in example_df.iterrows()])
    # example_desc = '\n'.join(['. '.join(df[df['area_id'] == row['area_id']].iloc[0].desc.split('\n')) + f'.\nAnswer: ' + str(round(row.score, 3)) + ' Billion KRW' for _, row in example_df.iterrows()])
    ...
    template = fill_in_templates(fill_in_dict, template)
    template += "\n\nIn your answer, the last line must always be in the format: \"Answer: X Million USD\"."
    # template += "\nDo not show your reasoning process. Answer the numeric score only."
...
def random_selection_nightlight(nl_json_path, n_shot, seed, target_ccode =False):
    ...
    # print(n_random_selection)
    return n_random_selection
def random_selection_GRDP(csv_path, n_shot, seed, target_ccode =False):
    #return : dictionary with {'selected_area_id':1, 'not_selected_area_id'}
    np.random.seed(seed)
    data = pd.read_csv(csv_path)
    data = data[data['GRDP'].notna()]
    data = data.sort_values(by='GRDP')
    # print(data)
    n = int((data.shape[0] / n_shot))
    # print(n)
    n_partition = [data.iloc[i:i+n] for i in range(0, data.shape[0], n)]
    # print(n_partition)
    if len(n_partition) > n_shot:
        n_end = n_partition.pop()
        n_partition[-1]=pd.concat([n_partition[-1], n_end])
    n_random_selection = [np.random.choice(x['area_id']) for x in n_partition]
    # print(data.loc[data['area_id'] ==n_random_selection[0]])
    # print(data.loc[data['area_id'] ==n_random_selection[1]])
    # print(data.loc[data['area_id'] ==n_random_selection[2]])
    # print(data.loc[data['area_id'] ==n_random_selection[3]])
    # print(data.loc[data['area_id'] ==n_random_selection[4]])
    # print(n_random_selection)
    return n_random_selection