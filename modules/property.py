...
from .helper import get_loc_geometry, get_ring_contained_loc, get_country, _get_y_x, get_user_comment
...
from .helper import ensure_air_proxy
...
def get_additional_comment(loc,config):
    """
    Provide comments for the given location.
    """
    # Pull admin/meta so comments are stable and readable
    ...
    return {
        "additional_comment": {
            "val": comment,
            "desc": "additional useful and private comments(MUST SELECT)",
            "type": "str",
            "weight": None
        }
    }
...
def get_air_info(
    ...
    if proxy_dir is None:
        proxy_dir = Path(cfg.path.proxy_dir)
    timeline = int(cfg.arcgis.timeline)
    ccode = cfg.arcgis.ccode
    ccode, adm1_name, adm2_name, areaid, loc_geom = get_adm2meta(loc,config)
    
    loc_data = get_ccode_loc(ccode)
    assert len(loc_data) > 0, f"No loc data for {ccode}"
    area_weight = loc_data[areaid]['weight']  

    seg_path = ensure_air_proxy(ccode=ccode, timeline=timeline, proxy_dir=proxy_dir)

    with open(seg_path, "r", encoding="utf-8") as f:
        proxy = json.load(f)

    if areaid not in proxy:
        return {
            "NO2_Sum":      {"val": 0.0, "desc": "Sum of tropospheric NO₂ column over the area (mol/m²)", "type": "default", "weight": None},
            "SO2_Sum":      {"val": 0.0, "desc": "Sum of SO₂ column over the area (mol/m²)",                 "type": "default", "weight": None},
            "CO_Sum":       {"val": 0.0, "desc": "Sum of CO column over the area (mol/m²)",                  "type": "default", "weight": None},
            "LST_Average":  {"val": 0.0, "desc": "Average land surface temperature (°C)",                    "type": "ratio",   "weight": area_weight},
        }

    node = proxy[areaid]
    def _g(node, first_class, second_class, default=0.0):
        
        return node[first_class][second_class]['val']

    no2_sum = _g(node, "no2", "NO2_Sum", default=0.0)
    so2_sum = _g(node, "so2", "SO2_Sum", default=0.0)
    co_sum  = _g(node, "co",  "CO_Sum",  default=0.0)
    lst_avg = _g(node, "lst", "LST_Average", default=0.0)

    result = {
        "NO2_Sum": {
            "val": str(round(no2_sum,3)) +'mol/m²',
            "desc": "Sum of NO₂ over the area",
            "type": "default",
            "weight": None
        },
        "SO2_Sum": {
            "val": str(round(so2_sum,3))+'mol/m²',
            "desc": "Sum of SO₂ over the area",
            "type": "default",
            "weight": None
        },
        "CO_Sum": {
            "val": str(round(co_sum,3)) +'mol/m²',
            "desc": "Sum of CO over the area",
            "type": "default",
            "weight": None
        },
        "LST_Average": {
            "val": str(round(lst_avg,1 )) +'°C',
            "desc": "Average land surface temperature",
            "type": "ratio",            
            "weight": area_weight
        },
    }
...
def get_poi_info(
    ...
    seg_path = proxy_dir / f'{ccode}_POI_{timeline}.json'
    # print(seg_path)
    loc_data = get_ccode_loc(ccode)
    assert len(loc_data)>0
    loc_weight = loc_data[areaid]['weight']
    # cols_POI = ['industiral_complex', 'production_facilities']
    if seg_path.exists():
        df = pd.read_json(seg_path).T
        df.index = df.index.astype(str)
        # print(areaid)
        # print(df.index)
        if areaid in df.index:
            # print("==========================================")
            
            POI_dict = df.loc[areaid, 'POI']
            new_dict={}
            # print(POI_dict)
            # print(POI_dict)
...
def get_adm2meta(loc, config):
    return point2adm2meta(loc, config)
...
def get_area(loc,config):
    val_desc = {
        'val':str(round(_point2adm2area(loc,config),1)) +'km²',
        'desc': 'Area of district of the given location',
        'type': 'default',
        'weight': None
    }
...
def get_night_light(
    ...
    if tif_path is None:
        tif_path = getattr(config.res_global_geotiff, curr_interest)
    if proxy_dir is None:
        proxy_dir = config.path.proxy_dir
    ...
    timeline = config.arcgis.timeline
    proxy_dir = Path(proxy_dir)
    seg_path = proxy_dir / f'{ccode}_{curr_interest}_{timeline}.json'
    loc_data = get_ccode_loc(ccode)
    assert len(loc_data)>0
    loc_weight = loc_data[areaid]['weight']
...
def get_distance_to_nearest_target(
    ...
    lat, lng = loc
    target_shp_path = getattr(config.res_global,target_name)
    col_name = config.res_global.col_name
    ...
    distance = str(round(min(distance_list),1)) +'km'
...
def get_population(
    ...
    ccode, adm1_name, adm2_name, areaid, loc_geom = get_adm2meta(loc,config)
    # country = get_country(ccode)
    pop_df_path = proxy_dir / f'{ccode}_population_{timeline}.csv'
