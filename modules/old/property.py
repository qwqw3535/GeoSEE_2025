...
def get_additional_comment(loc):
    """
...
def get_air_info(
    ...
    timeline = _CONFIG.arcgis.timeline
    ...
    if seg_path.exists():
        ...
        if areaid in df.index:
            target_row = df.loc[areaid]
            result_dict = {}
            # print(target_row)
            result_dict['NO2_Sum'] = target_row['no2']['NO2_Sum']
            result_dict['SO2_Sum'] = target_row['so2']['SO2_Sum']
            result_dict['CO_Sum'] = target_row['co']['CO_Sum']
            result_dict['LST_Average'] = target_row['lst']['LST_Average']
            # print(result_dict)
            return result_dict
    else:
        data = {}
...
def get_poi_info(
    ...
    if seg_path.exists():
        df = pd.read_json(seg_path).T
        if areaid in df.index:
            target_row = df.loc[areaid,'industry']
            result_dict = {}
            for key, val in target_row.items():
                count_info = val["Count"]
                count_info["desc"] = f"Total count of '{key}' buildings in the category"
                result_dict[key] = count_info
            return result_dict
    else:
        data = {}
    return result_dict
